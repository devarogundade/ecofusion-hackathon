...
const VERRAProgress = () => {
  ...
  const progressPercentage = activeRound 
    ? Math.round((activeRound.achieved_amount / activeRound.target_amount) * 100)
    : 0;
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        {activeRound && (
          <div className="p-4 rounded-lg bg-accent/5 border border-accent/20">
            <div className="flex items-center justify-between mb-2">
              <span className="font-semibold">Round {activeRound.round_number} - Active</span>
              <Badge variant="default" className="bg-accent">In Progress</Badge>
            </div>
                ...
                <div className="p-4 rounded-lg border border-border bg-card h-full">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">${round.target_amount.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Achieved:</span>
                      <span className="font-medium">${round.achieved_amount.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      ...
                      <Badge 
                        ...
                        className={round.status === 'completed' ? 'bg-accent' : ''}
                      >
        ...
        <div className="p-4 rounded-lg bg-secondary/30 text-sm text-muted-foreground">
          <p className="mb-2">
            <strong>How it works:</strong> Fractionalized carbon credit tokens are distributed each round. 
            Once a round reaches its target, the full carbon credit is certified by VERRA, and tokens become tradeable.
          </p>
          <p>
            Completed rounds have been officially certified, giving your tokens real environmental value backed by VERRA standards.
          </p>
        </div>
      </CardContent>
    </Card>
